...
const QuickActionButton = ({ icon: Icon, label, color, onClick }: Props) => (
  <button onClick={onClick} className={`p-2 sm:p-3 lg:p-3 rounded-lg sm:rounded-xl ${colors[color]} transition-colors flex flex-col items-center gap-1 sm:gap-1.5`}>
    <Icon className="w-4 h-4 sm:w-5 sm:h-5"/><span className="text-[10px] sm:text-xs font-semibold text-center leading-tight">{label}</span>
  </button>
);
