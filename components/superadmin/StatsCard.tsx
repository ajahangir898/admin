...
const StatsCard = ({ title, value, change, changeType, icon: Icon, iconBg, iconColor, subtitle }: Props) => (
  <div className="bg-white rounded-xl p-3 sm:p-4 shadow-sm border border-slate-200 hover:shadow-md transition-shadow">
    <div className="flex items-start justify-between gap-2">
      <div className="min-w-0 flex-1">
        <p className="text-[10px] sm:text-xs lg:text-sm font-medium text-slate-500 truncate">{title}</p>
        <p className="text-base sm:text-xl lg:text-2xl font-bold text-slate-900 mt-0.5 truncate">{value}</p>
      ...
      <div className={`w-8 h-8 sm:w-10 sm:h-10 lg:w-10 lg:h-10 ${iconBg} rounded-lg sm:rounded-xl flex items-center justify-center flex-shrink-0`}><Icon className={`w-4 h-4 sm:w-5 sm:h-5 lg:w-5 lg:h-5 ${iconColor}`}/></div>
    </div>
  </div>
);
