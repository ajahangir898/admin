...
}> = ({ product, onClick, variant, onQuickView, onBuyNow, onAddToCart }) => {
        ...
        <div className="px-2 py-1.5 flex-1 flex flex-col">
          ...
          <div className="mt-auto">
            ...
            <div className="flex gap-1.5">
              <button className="flex-1 btn-order py-1 text-xs" onClick={handleBuyNow}>
                Buy Now
              </button>
              <button
                type="button"
      ...
      <div className="bg-white rounded-lg border border-gray-200 hover:shadow-lg transition-all duration-300 group relative overflow-hidden flex flex-col">
        ...
        <div className="px-4 pt-2 pb-4 flex-1 flex flex-col">
          ...
          <div className="flex items-center gap-2 mb-4">
            <span className="text-base font-bold" style={{ color: 'rgba(var(--color-primary-rgb), 1)' }}>
              ৳{product.price}
            </span>
            {product.originalPrice && (
          ...
          <div className="flex flex-col gap-2 mt-auto">
            <button className="w-full text-white text-xs font-bold py-2 rounded-md transition btn-order" onClick={handleBuyNow}>
              Buy Now
            </button>
            <button 
              className="w-full text-xs font-bold py-1.5 rounded-md transition border-2"
              style={{ borderColor: 'rgba(var(--color-primary-rgb), 1)', color: 'rgba(var(--color-primary-rgb), 1)', backgroundColor: 'transparent' }}
              onMouseEnter={(e) => { e.currentTarget.style.backgroundColor = 'rgba(var(--color-primary-rgb), 0.1)'; }}
    ...
    <div className="group relative flex flex-col rounded-2xl border border-gray-100 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-sm hover:shadow-2xl transition-all duration-500 overflow-hidden">
      ...
      <div className="p-4 flex-1 flex flex-col gap-3">
        <div>
          ...
          <h3 className="font-bold text-gray-900 dark:text-gray-100 text-base leading-tight cursor-pointer hover:text-emerald-600 dark:hover:text-emerald-300 transition line-clamp-2" onClick={() => onClick(product)}>
            {product.name}
          </h3>
          {product.description && (
        ...
        <div className="mt-auto flex items-end justify-between gap-3">
          ...
          <div className="flex flex-col gap-2 w-32">
            <button onClick={handleBuyNow} className="w-full btn-order py-2 rounded-xl font-bold text-sm">
              অর্ডার করুন
            </button>
            <button
              type="button"
            ...
            <button
              type="button"
              onClick={(e) => { e.stopPropagation(); if (onQuickView) onQuickView(product); else onClick(product); }}
